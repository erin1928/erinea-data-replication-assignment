...
```
In the paper, researchers compare the dates in which samples are collected and the colonies bees are from. In order to begin this process let's create a "dates" vector using the summary function to isolate it form our data set.
```{r dates, include=TRUE}
dates <- summary(bee_data$dat) #using summary function to call and name our dates
dates
length(dates) #looking at the lengths of dates
length(colony)
```
Now, using a generalized linear mofel we can compare the date of collection to the colony the bee came from.
```{r GLM, include=TRUE}
glm <- glm(data = bee_data, dates ~ colony, family = "binomial") #create the glm
summary(glm) #call the results
```
According to the paper, "a preliminary analysis showed that the variance explained by the colony ID and area was numerically zero, and hence was not included as random factors." Here, our p-value column is much higher than 0.05, and we can confirm this interaction is not statistically significant.
